...
function help() {
  ...
  text("select a critter\nUse arrow keys to change population\n↑↓ quick change\n←→ slow change", menuX + 180, menuY + 175)
  textAlign(RIGHT)
...
class Shrimpy extends Flock {
  ...
  display() {
    ...
    drawShrimpy(this.size, this.color)
    pop()
  }
}

function drawShrimpy(size, color) {
  let opacity = 100
  if (simple) {
    fill(color[0], color[1], color[2], 150 + opacity)
    push()
    noStroke()
    rectMode(CENTER)
    rect(0, 10, 4, 20)
    ...
    push()
    fill( color[0], color[1], color[2], 80 + opacity)
    noStroke()
    beginShape()
    vertex(-8*size, 0)
